import { ArrowDown, ArrowUpRight, Check, MapPin, MessageSquare, Phone, Trees } from "lucide-react";
import type { Metadata } from "next";
import { business, services, steps } from "@/content/site";
...
export default function Home() {
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <header className="header wrap">
        ...
        <nav className="desktop-nav" aria-label="Main navigation"><a href="#services">What we do</a><a href="#about">Who we are</a><a href="#neighbors">Kind words</a></nav>
    ...
    <section className="process-section section wrap"><div className="process-heading reveal"><p className="eyebrow">03 / SIMPLE FROM THE START</p><h2>Less on your list.<br /><em>More room to breathe.</em></h2></div><div className="steps">{steps.map(step => <article className="step reveal" key={step.number}><span>{step.number}</span><h3>{step.title}</h3><p>{step.description}</p></article>)}</div></section>
    <Testimonials />
